...
export const AgentsDatabaseView: React.FC<AgentsDatabaseViewProps> = ({
  ...
  // Credentials & Temporary Passwords viewing (Staff & Admin)
  const [credentialsTargetAgent, setCredentialsTargetAgent] = useState<AgentProfile | null>(null);
  const [visibleTempPasswords, setVisibleTempPasswords] = useState<Record<string, boolean>>({});

  const toggleTempPasswordVisibility = (code: string) => {
    setVisibleTempPasswords((prev) => ({
      ...prev,
      [code]: !prev[code],
    }));
  };
  ...
  // Export to 34-Column Official Dataset (CSV)
  const handleExportCSV = () => {
    download34ColumnCsv(agents);
    setNotificationToast({
      message: `Exported ${agents.length} agent records to 34-column CSV file.`,
      type: 'success',
    });
  };

  // Export to 34-Column Official Dataset (Excel .xlsx)
  const handleExportExcel = () => {
    download34ColumnExcel(agents);
    setNotificationToast({
      message: `Exported ${agents.length} agent records to 34-column Microsoft Excel (.xlsx) file.`,
      type: 'success',
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-700" /> Export 34-Col Excel (.xlsx)
            </button>
                ...
                {filteredAgents.map((agent, index) => {
                  ...
                  return (
                    <tr key={agent.affiliateCode} className="hover:bg-blue-50/40 transition">
                      ...
                      <td className="py-2 px-3 border-r border-slate-100 font-sans bg-amber-50/30">
                        <div className="flex items-center gap-1.5">
                          <span className="font-mono font-bold text-slate-900 bg-amber-100/70 px-1.5 py-0.5 rounded border border-amber-300 text-[10px]">
                            {visibleTempPasswords[agent.affiliateCode]
                              ? agent.tempPassword || `Mega@${agent.affiliateCode.split('-').pop() || '2026'}`
                              : '••••••••'}
                          </span>
                          <button
                            type="button"
                            onClick={() => toggleTempPasswordVisibility(agent.affiliateCode)}
                            className="text-slate-400 hover:text-slate-700 p-0.5 rounded hover:bg-slate-100 transition"
                            title={visibleTempPasswords[agent.affiliateCode] ? 'Hide Password' : 'Show Temporary Password'}
                          >
                            {visibleTempPasswords[agent.affiliateCode] ? (
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setNotificationToast({
                                message: `Copied temporary password for ${agent.fullName}: ${pass}`,
                                type: 'success',
                              });
                            }}
                            className="text-slate-400 hover:text-blue-900 p-0.5 rounded hover:bg-slate-100 transition"
                      ...
                      <td className="py-2 px-3 text-center font-sans">
                        <div className="flex items-center justify-center gap-2">
                          {/* Credentials & Temporary Password modal trigger */}
                          <button
                            type="button"
                            onClick={() => setCredentialsTargetAgent(agent)}
                            className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-800 hover:text-amber-950 bg-amber-50 hover:bg-amber-100 px-2 py-0.5 rounded border border-amber-200 transition"
                            title="View Temporary Password & Send Renewal Credentials"
                          >
      ...
      {/* Agent Credentials & Temporary Password Modal */}
      <AgentCredentialsModal
        isOpen={Boolean(credentialsTargetAgent)}
        onClose={() => setCredentialsTargetAgent(null)}
        agent={credentialsTargetAgent}
        currentUser={currentUser}
        showToast={(msg) => setNotificationToast({ message: msg, type: 'info' })}
        onUpdateAgent={(updated) => {
          setCredentialsTargetAgent(updated);
        }}
      />
    </div>
  );
